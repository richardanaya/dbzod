...
const GENERAL_HELP = `database-validator-generator

Generate Zod schemas and JSDoc typedefs from database metadata.

Usage:
  database-validator-generator generate [options]
  database-validator-generator list tables [options]
  dbvg generate [options]
  dbvg list tables [options]
...
const GENERATE_HELP = `database-validator-generator generate

Generate Zod schemas and JSDoc typedefs from PostgreSQL tables.

Usage:
  database-validator-generator generate --connection <postgres-url> [--schema public] [--out schemas.mjs]
  DATABASE_URL=<postgres-url> database-validator-generator generate
...
const LIST_TABLES_HELP = `database-validator-generator list tables

List PostgreSQL tables in a schema.

Usage:
  database-validator-generator list tables --connection <postgres-url> [--schema public]
  DATABASE_URL=<postgres-url> database-validator-generator list tables
...
function parseArgs(argv) {
  ...
  if (command === "list") {
    ...
    if (subcommand !== "tables") {
      throw new Error("Unknown command. Use `database-validator-generator list tables` or `database-validator-generator --help`.");
    }
  ...
  throw new Error(`Unknown command: ${command}. Use \`database-validator-generator --help\`.`);
}
...
function parseOptions(argv, { allowOut }) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "-o" || arg === "--out") {
      if (!allowOut) {
        throw new Error(`${arg} is only supported by database-validator-generator generate`);
      }
...
function applyStringFormatChecks(expression, column) {
  ...
  if (/@(?:(?:dbzod|database-validator-generator)\s+)?format\s+email\b/i.test(comment)) {
    return `${expression}.email()`;
  }

  if (/@(?:(?:dbzod|database-validator-generator)\s+)?format\s+(?:ip|inet)\b/i.test(comment) || column.udtName === "inet") {
...
function getZodDirective(description) {
  ...
  const match = String(description).match(/@(?:(?:dbzod|database-validator-generator)\s+)?zod\s+([^\r\n]+)/i);
...
async function checkGeneratedFile(path, expectedOutput) {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      throw new Error(`${path} does not exist. Run database-validator-generator generate first.`);
    }
  ...
  if (existingOutput !== expectedOutput) {
    throw new Error(`${path} is stale. Run database-validator-generator generate to update it.`);
  }
}
